...
class InvalidSettingTypeError(Exception):
    def __init__(self, setting, expected_type, actual_type):
        super().__init__('Wrong type for {}: expected `{}`, got `{}`'.format(setting, expected_type.__name__, actual_type.__name__))
...
class SettingValueNotAllowedError(Exception):
    def __init__(self, setting, value, valid_entries):
        super().__init__('Value for {} should be one of {}'.format(value, str(valid_entries)))
...
Settings.declare_setting('mode', 'manual', valid_entries=['manual', 'heartrate', 'speed'], valid_type=str)
...
Settings.declare_setting('zwift_userid', None, valid_type=int)
Settings.declare_setting('speed_thresholds', [15,25,35], valid_type=list)
Settings.declare_setting('physical_buttons', 'none', valid_type=str, valid_entries=['none', 'up-down', 'speeds'])
Settings.declare_setting('buttons_gpio', {}, valid_type=dict)
